...
/**
 * [applyVariables Replace variable with actual value]
 * @param  {[Object|String]} content   [Content going to parse]
 * @param  {[Object]} variables [user defined variables]
 * @return {[Object|String]}           [Parsed contents]
 */
...
describe('json-template', () => {
  ...
  describe('functions', () => {
    ...
    const result = applyVariables(userCard, { city: 'Surat' });

    it('should compute template with Function', function () {
      assert.notDeepEqual(
        {
          uuid: '9125a8dc-52ee-365b-a5aa-81b0b3681cf6',
          name: 'John',
          city: 'Mumbai',
        },
        result
      ...
      assert.match(result.uuid, v4Regex);
    });
  });
  ...
  describe('no match on the given context', () => {
    ...
    it('should handle multi-value expressions where the first value is null, but has a defaultValue', () => {
      const template = parse({
        boo: '{{foo.isNull:defaultValue}} {{foo.isNonNull}}',
      });
      assert.deepEqual(
        template({ foo: { isNull: null, isNonNull: 'value' } }),
        { boo: 'defaultValue value' }
      );
    });
  });
});
